...
class _TaskHomePageState extends State<TaskHomePage> {
  ...
  _generateData(mydata) {
    ...
    _seriesPieData.add(
      charts.Series(
        domainFn: (Task task, _) => task.Nationality,
        measureFn: (Task task, _) => task.Cases,
        colorFn: (Task task, _) =>
            charts.ColorUtil.fromDartColor(Color(int.parse(task.Color))),
        id: 'Nationality',
        data: mydata,
        labelAccessorFn: (Task row, _) => "${row.Cases}",
      ),
    );
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.blueGrey[200],
          title: Text('Nationality', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold),)),
  ...
  Widget _buildBody(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: Firestore.instance.collection('Nationality').snapshots(),
